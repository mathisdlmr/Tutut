...
Exemple (simplifié) :
````php
Route::get('/RGPD', [\App\Http\Controllers\RGPDController::class, 'index']);
````

**Traduction :**  
Quand quelqu’un va sur `/RGPD`, Laravel appelle la méthode `index` du contrôleur `RGPDController`.
